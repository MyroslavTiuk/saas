...
const ReportsWeeklyAttendance = () => {
  ...
  const {data: attendance, isFetching} = useQuery([
    ...
    queryFn: async () => {
      ...
      const result = await new API().getAttendanceWeekly(queryString);
      if (result != null) {
        return result;
      }
      return [];
  ...
  const dates = useMemo(() => {
    ...
    const momentDate = moment(attendance.weekload.start_at).add(-1, "days");
    const length = 7;
    return Array.from({length}).map(() => momentDate.add(1, "days").clone());
  }, [attendance]);

  const getGeneralReportURL = () => {
    let url = Config.api_url + "/v1/admin/report/print_weekly?date=" + getWeekDay(inputDate);
  ...
  const getEmployeeReportURL = (row) => {
    let url = Config.api_url + "/v1/admin/report/employee/print_weekly?date=" + getWeekDay(inputDate);
      ...
      renderCell: ({row}) => (
        <div className="cellAction">
          <EmployeeReportLink
            ...
            filename={`weekly_${row.name_report}`}
          >
            Print Report
          </EmployeeReportLink>
        </div>
          ...
          <div className="reportsContainer">
            <div className="reportsSection">
              <div className="datatable">
                ...
                <div className="my-2">
                  <span>{attendance ? `${moment(attendance.weekload.start_at).format("DD/MM/YYYY")} ~ ${moment(attendance.weekload.end_at).format('DD/MM/YYYY')}` : ''}</span>
                </div>
                <div className="d-flex justify-content-between">
                  <div className="d-flex justify-content-start gap-3">
                    ...
                    <AuthenticatedLink url={getGeneralReportURL()} filename={`print_weekly_${inputDate}`}>
                      Print Report
                    </AuthenticatedLink>
                  </div>
                  <input
...
export default ReportsWeeklyAttendance;
